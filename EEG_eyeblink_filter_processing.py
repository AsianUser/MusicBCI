...
def load_csv_eeg_subject_windows(subject_index, window_length=0.5, tmin=0.0, tmax=0.5):
    ...
    data_path = os.path.join(DATA_DIR, f"{subj}_data.csv")
...
subj_per_epoch = np.concatenate([
    np.full(len(ep), fill_value=i, dtype=int)
    for i, ep in enumerate(subject_epochs)
])
...
random_guess_train = {
    name: np.mean(y_train == code)
    for name, code in zip(class_names, class_codes)
}
print("Random guess probabilities (train):", random_guess_train)

random_guess_test = {
    name: np.mean(y_test == code)
    for name, code in zip(class_names, class_codes)
}
